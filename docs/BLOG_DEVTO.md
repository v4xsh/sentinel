---
title: "Sentinel: an agentic fraud investigator on TigerGraph"
published: false
tags: ai, graphdatabase, python, machinelearning
...
![Cases list](https://raw.githubusercontent.com/v4xsh/sentinel/main/docs/img/cases.png)
...
HHG-014 is the flagship. An analyst-flagged case where several cards in the month share an unusual device profile. The T4 device signal (a narrow shared device with a prior confirmed-fraud ClosedCase attached to it) fires `shared_element = "device"`. §3a fires the SAR. R6 fires `MONITOR_CONNECTED_CARDS`. The initial actions are `STEP_UP_AUTH`, `CREATE_CASE`, `FILE_REPORT` because the §6 two-channel gate cleared 0.85 before the customer was even contacted. When the customer denies (or the simulator denies for them at τ = 0.30), the final adds `BLOCK_CARD`.

![HHG-014 graph neighbourhood](https://raw.githubusercontent.com/v4xsh/sentinel/main/docs/img/hhg014_graph.png)

The Graph tab above shows Sentinel's live view of HHG-014: the case in the middle (red), the shared `SM-G935F Build/NRD90M | Android 7.0 | chrome 62.0 for android` device (yellow), and the **19 other cards on that exact device_profile in the ±14-day window (blue)** with the **4 confirmed-fraud ClosedCases attached to that device (purple, CC-2649, CC-2971, CC-2985, CC-3035)**. The case's own evidence quotes a broader **74 other cards** — that number comes from `ring_components` with `p_expand_family=True`, which follows the whole device family, not just the exact profile string. Both counts are true; the graph view shows the strict same-profile neighbourhood, the evidence quotes the broader family.
...
![Backtest report](https://raw.githubusercontent.com/v4xsh/sentinel/main/docs/img/backtest.png)
